...
// ── compute everything term-relative ──
function computeModel(data) {
  ...
  const curSearchCombined = prod.billable_search_requests + stg.billable_search_requests;
  const termSearch = curSearchCombined - baseline;            // searches used THIS term
  ...
  const recordsCombined = prod.billable_records + stg.billable_records; // monthly measure, not cumulative
  const apps = prod.period_end_live_apps;                     // prod parent only

  // pacing + projection (search)
  ...
  const fullTermProj = Math.round(ratePerDay * termDays);
  ...
  const appTot = seg.prod + seg.nonprod + seg.legacy || 1;
  const split = {
  ...
  return {
    ...
    recordsCombined, recordsPct: pct(recordsCombined, c.records_quota), recordsQuota: c.records_quota,
    ...
    elapsedPct: pct(elapsedDays, termDays), exhaustLabel, fullTermProj,
    fullTermPct: pct(fullTermProj, c.searches_quota), remaining,
    prodSearch: prod.billable_search_requests, stgSearch: stg.billable_search_requests,
    prodRecords: prod.billable_records, stgRecords: stg.billable_records, stgApps: stg.period_end_live_apps,
    split, eng, engTotal,
  };
}
...
function buildEmailBody(data) {
  ...
  // Search color reflects the OUTLOOK, not just the point-in-time %: if it's projected to
  // exceed the quota by term end, it's a watch (amber) even when current usage is < 70%.
  const searchColor = m.searchPct >= 100 ? '#dc2626'
    : (m.searchPct >= 85 || m.fullTermPct >= 100) ? '#d97706'
    : '#16a34a';
...
<tr><td style="padding:16px 22px 4px">
  ...
  <div style="font-size:14.5px;line-height:1.5;color:#1a1d35;font-weight:600">The one to watch is <span style="color:${searchColor}">search</span> — at the current pace it exhausts this term's full allowance well before renewal (see below), with staging/test driving the bulk of the burn. Apps are at the cap; records are comfortable.</div>
</td></tr>
...
<tr><td style="padding:14px 22px 4px">
  <table width="100%" cellpadding="0" cellspacing="0"><tr>
    <td width="42%" valign="top" style="padding-right:16px;border-right:1px solid #eef0f4">
      ...
      ${quotaRowNarrow('Records', m.recordsPct, `${fmtM(m.recordsCombined)} / 50M`, '#16a34a')}
      ${quotaRowNarrow('Applications', m.appsPct, `${m.apps.toLocaleString()} / ${m.appsQuota.toLocaleString()}`, '#dc2626')}
    </td>
...
<!-- SEARCH OUTLOOK (full width) -->
<tr><td style="padding:6px 22px 4px">
  <div style="font-size:11px;font-weight:600;color:#9698C3;letter-spacing:0.8px;margin:6px 0 8px;border-top:1px solid #f3f4f6;padding-top:12px">SEARCH OUTLOOK</div>
  <table width="100%" cellpadding="0" cellspacing="0" style="font-size:12.5px;color:#36395A">
    <tr><td style="padding:3px 0">Used this term</td><td style="text-align:right;font-weight:600;color:#000033">${fmtM(m.termSearch)} of 75M (${m.searchPct.toFixed(0)}%)</td></tr>
    <tr><td style="padding:3px 0">Term elapsed</td><td style="text-align:right;color:#000033">~${m.elapsedPct.toFixed(0)}% — so usage is ahead of pace</td></tr>
    <tr><td style="padding:3px 0">Remaining</td><td style="text-align:right;font-weight:600;color:#000033">${fmtM(m.remaining)}</td></tr>
    <tr><td style="padding:3px 0">On pace to exhaust</td><td style="text-align:right;font-weight:700;color:${searchColor}">~${m.exhaustLabel}</td></tr>
    <tr><td style="padding:3px 0">Full-term projection</td><td style="text-align:right;font-weight:600;color:#000033">~${fmtM(m.fullTermProj)} (${m.fullTermPct.toFixed(0)}% of quota)</td></tr>
    <tr><td style="padding:3px 0">Staging vs production</td><td style="text-align:right;color:#000033">staging ${fmtM(m.termSearchStg)} · prod ${fmtM(m.termSearchProd)}</td></tr>
  </table>
  <div style="font-size:11px;color:#7778AF;margin-top:6px;line-height:1.45">Staging/test is ~${m.stgShareOfBurn.toFixed(0)}% of the search burned this term — worth a look on its own.</div>
</td></tr>
...
<tr><td style="padding:14px 22px 6px">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e9ebf2;border-radius:6px;font-size:12.5px;color:#36395A">
    ...
    <tr style="border-top:1px solid #e5e7eb">
      ...
      <td style="text-align:right;padding:8px;font-weight:700;color:#000033">${fmtM(m.recordsCombined)}</td>
  ...
  <div style="font-size:10.5px;color:#9698C3;margin-top:8px;line-height:1.45">Indented rows show each segment's share of the Production parent's active child apps. Staging / Test is a separate parent account (search &amp; records count toward the combined quota; its app count is tracked separately).</div>
</td></tr>
...
if (require.main === module) {
  ...
  console.log('\nKey numbers (term-relative, from data):');
  ...
  console.log('          on pace to hit 75M ~' + m.exhaustLabel + ', full-term proj ~' + fmtM(m.fullTermProj) + ` (${m.fullTermPct.toFixed(0)}%)`);
  console.log('          staging share of burn:', m.stgShareOfBurn.toFixed(0) + '%', `(stg ${fmtM(m.termSearchStg)} / prod ${fmtM(m.termSearchProd)})`);
  console.log('  RECORDS:', m.recordsCombined.toLocaleString(), `= ${m.recordsPct.toFixed(1)}% of 50M (combined)`);
